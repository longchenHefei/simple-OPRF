...
// bool YaoEvaluator::Init_receive(Player& P)
// {

// 	P.receive_player(1, Initfinalize);
// 	return true;
// }
...
//test only communication
	...
	//b = GC::TIME_BREAK;
	//printf("GC is done.j = %d\n",j);
		printf("GC is done. This is Garbler.\n");
	//}
		// Receive in Init.
		// P->receive_player(1, Initfinalize);

			//copy comout to Init_pack
		// printf("Server reconstruct. n = %d\n", n);
		// for(int i = 0; i < n + 2; i++)
		// 	init_128[i] = G.get_doubleword();

		// for (int j = 0; j < n + 2; j++){
		// 	_mm_storeu_si128(reinterpret_cast<__m128i*>(bytes2), init_128[j+1]);
		// 	for (int i = 0; i < 16; i++) {
...
void YaoGarbler::process_receiver_inputs()
{
	...
	while (not receiver_input_keys.empty())
	{
		...
		BitVector _;
		//  printf("This is a Garbler____________________.\n");
