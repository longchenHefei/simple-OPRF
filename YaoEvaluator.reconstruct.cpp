...
//	unsigned char Init_pack[ 16 * n + 32 ];
	...
	// PPSS Reconstruction, res[i][j] = l_i(x_j), i \in [1, t+1], j \in [0, n]
	__m128i res[t + 2][ n + 1];
	...
	for (int j = 0; j < n; j++){
		...
		//reconstruction, s_i = e_i XOR rou_i
		shares[j + 1] = _mm_xor_si128(e[j + 1], oprf_128[j + 1]);
	}
...
*/
	//reconstruction, shares_1, ..., shares_t, shares_(t+1)

	P.receive_player(0, Initfinalize);

	printf("Clients starts to reconstruct.\n");

	// Lagrange Basis Function l_i(target) = res = 1
	for( int target = 0; target < n + 1; target++){
		for(int i = 0; i < t + 2; i++){
			res[i][target] = _mm_setr_epi32(1, 0, 0, 0);
		}
	}

	// l_i(target) = prod{ (target - x_j) /(x_i - x_j) }, l_i(x_i) == 1, l_j(x_i) == 0
	// l_i(target) = res[i][target] = prod{ (points_j - target) / (points_j - points_i) }
	// no need to compute l_i(x_1, ..., x_t, x_(t+1)), target == 0 || target > t+1, target \in {0} and {t+2 ,..., n}
	// i \in [1, t+1], j \in [1, t+1]
			...
			for(int j = 1; j < t + 2; j++){
				if(j != i){
					if(target == 0 || target > t + 1){
						res[i][target] *= (points[j] - target) / (points[j] - points[i]);
						printf("compute res[%d][%d], j=%d.\n", i, target, j );
					}
				}
			} 
		}
	...
	printf(" res done. \n");
	// Shares[target] = sum_{i \in [1, t+1]} (shares[i] l_i[target]), target \in {0} and {t+2 ,..., n}
	for( int target = 0; target < n + 1; target++){
		if(target == 0 || target > t + 1){
			shares[target] = _mm_setzero_si128();
		}
	}
	...
	for( int target = 0; target < n + 1; target++){
		if(target == 0 || target > t + 1){
			for(int i = 1; i < t + 2; i++){
				shares[target] +=  shares[i] * res[i][target] ;
			}
		}
	}

	//reconstruct s, s_{t+2},...,s_n
	secret = shares[0];
